...
def guide_guide_eff(
    ...
    if config == "partial_pooling":
        guide_eff_mean_loc = numpyro.param(
            "guide_eff_mean_loc", jnp.full((lengths["len_guides"],), mean_l)
        )
        ...
        guide_eff_std_loc = numpyro.param(
            "guide_eff_std_loc", jnp.full((lengths["len_guides"],), std_l)
        )
        ...
        with numpyro.plate("cell_lines", lengths["len_cell_lines"]):
            with numpyro.plate("guides_per_cell", lengths["len_guides"]):
                ...
                guide_eff_ = (
                    guide_eff_mean_[:, None] + guide_eff_std_[:, None] * tilde_alpha
                )
        ...
        guide_eff_scale = numpyro.param(
            "guide_eff_scale",
            jnp.full(
                (lengths["len_cell_lines"], lengths["len_guides"]),
                jnp.abs(mean_s) + 1e-3,
            ),
            constraint=dist.constraints.positive,
        )
            ...
            with numpyro.plate("guides", lengths["len_guides"]):
                numpyro.sample(
                    ...
                    dist.TruncatedNormal(
                        guide_eff_loc, guide_eff_scale, low=0.0, high=1.0
                    ),
                )

    elif config == "full_pooling":
        guide_eff_single_loc = numpyro.param(
            "guide_eff_single_loc", jnp.full((lengths["len_guides"],), mean_l)
        )
        ...
        raise ValueError(
            "Invalid guide_config: must be 'partial_pooling', 'no_pooling', or 'full_pooling'."
        )
...
def guide_cell_line(lengths, prior_params, zi=False):
    ...
    cell_line_growth_loc = numpyro.param(
        "cell_line_growth_loc", jnp.full((lengths["len_cell_lines"],), growth_cell_l)
    )
    ...
    with numpyro.plate("cell_lines", lengths["len_cell_lines"]):
        ...
        library_bias_loc = numpyro.param(
            "library_bias_loc", jnp.ones(lengths["len_cell_lines"])
        )
        ...
        numpyro.sample(
            "library_bias", dist.Normal(library_bias_loc, library_bias_scale)
        )
        # Turn this on for now, but not used in the model
        # if zi:
        p_zi_loc = numpyro.param("p_zi_loc", 0.05 * jnp.ones(lengths["len_cell_lines"]))
        ...
        numpyro.sample(
            "p_zi", dist.TruncatedNormal(p_zi_loc, p_zi_scale, low=0.0, high=1.0)
        )
...
def guide_global_overdisp(lengths, prior_params):
    ...
    mv_cell_line_loc = numpyro.param(
        "mv_cell_line_loc", jnp.array(od_means, dtype=jnp.float32)
    )
...
def guide_gene_std(lengths, prior_params):
    ...
    gene_std_scale = numpyro.param(
        "gene_std_scale", mv_mean_s, constraint=dist.constraints.positive
    )
    # with numpyro.plate("gene_std_plate", lengths["len_cell_lines"]):
    numpyro.sample("gene_std", dist.HalfNormal(gene_std_scale))
...
def guide_gene_ko_growth(lengths, prior_params):
    if "gene_effect_means" not in prior_params:
        ...
        gko_loc = numpyro.param(
            "gene_ko_growth_loc", jnp.full((lengths["len_genes"],), growth_l)
        )
        ...
        with numpyro.plate("genes", lengths["len_genes"]):
            numpyro.sample("gene_ko_growth", dist.Normal(gko_loc, gko_scale))
    else:
        ...
        # with numpyro.plate("genes", lengths["len_genes"]):
        numpyro.sample("gene_ko_growth", dist.Normal(gko_loc, gko_scale).mask(mask))


def guide_single_od(lengths):
    ngene_loc = numpyro.param(
        "non_centered_deviation_gene_loc", jnp.zeros(lengths["len_genes"])
    )
...
def guide_double_od(lengths, prior_params):
    ...
    ndp_loc = numpyro.param(
        "non_centered_deviation_loc", jnp.zeros(lengths["len_gene_pairs"])
    )
    ...
    with numpyro.plate("gene_pairs", lengths["len_gene_pairs"]):
        numpyro.sample("non_centered_deviation", dist.Normal(ndp_loc, ndp_scale))
...
def guide_pair_growth(lengths, prior_params):
    ...
    with numpyro.plate("gene_pairs", lengths["len_gene_pairs"]):
        numpyro.sample("gene_pair_ko_growth", dist.Normal(pg_loc, pg_scale))


def guide_control_od(lengths):
    ndc_loc = numpyro.param(
        "non_centered_dev_c_loc", jnp.zeros(lengths["len_guide_pairs_c"])
    )
...
def guide_init_counts_double(lengths, prior_params):
    ...
    ic_loc = numpyro.param(
        "guide_init_count_loc", jnp.full((lengths["len_guide_pairs"],), init_l)
    )
    ...
    with numpyro.plate("guide_counts", lengths["len_guide_pairs"]):
        numpyro.sample(
            "guide_init_count", dist.TruncatedNormal(ic_loc, ic_scale, low=0.0)
        )
...
def guide_init_counts_single(lengths, prior_params):
    ...
    ics_loc = numpyro.param(
        "guide_init_count_s_loc", jnp.full((lengths["len_guide_pairs_s"],), init_s_l)
    )
    ...
    with numpyro.plate("guides_counts_s", lengths["len_guide_pairs_s"]):
        numpyro.sample(
            "guide_init_count_s", dist.TruncatedNormal(ics_loc, ics_scale, low=0.0)
        )
...
def guide_init_counts_control(lengths, prior_params):
    ...
    icc_loc = numpyro.param(
        "guide_init_count_c_loc", jnp.full((lengths["len_guide_pairs_c"],), init_c_l)
    )
    ...
    with numpyro.plate("guides_counts_c", lengths["len_guide_pairs_c"]):
        numpyro.sample(
            "guide_init_count_c", dist.TruncatedNormal(icc_loc, icc_scale, low=0.0)
        )


def guide_init_counts(
    lengths, prior_params, no_singletons, only_singletons, no_controls
):
